...
function handleLookup(query, doneCallback) {
	...
	for(let i=0;i<CacheData.length;i++){
		if(CacheData[i].query === query){
			found = true;
			doneCallback( { suggestions: CacheData[i].result } );
		}
	}
	if(found===false){
...
// TODO: jump to the specific result page based on the selected suggestion
	...
	var url = suggestion["data"]["category"]  + "?id=" + suggestion["data"]["heroID"]
	console.log(suggestion["data"]["category"])
	if(suggestion["data"]["category"]==="----------Movie----------")
	{
		url= "MoviePage.html?title_first_character="+suggestion["value"];
		window.location.href=url;
	}
	else
	{
		url= "StarPage.html?star_name="+suggestion["value"];
		window.location.href=url;
	}
}
...
function handleNormalSearch(query) {
	...
	let url="Movie_List.html?";
	url= url+"title="+escape(query);
	window.location.href=url; 
}
